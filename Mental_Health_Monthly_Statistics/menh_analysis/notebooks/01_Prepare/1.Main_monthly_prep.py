...
# DBTITLE 1,CC0011_PREP added to clean up analyst code that makes the MHS29d_prep, MHS29d_prov_prep,MHS29_f
 %sql
 --In the original analyst code, they use the $db_source.MHS201CareContact table itself as primary source table, therefore a long hardcoded list is added to make up the AttendStatus.
 CREATE OR REPLACE GLOBAL TEMP VIEW CC001_Prep as
  ...
  ,CC.UniqCareContID
  ,CASE
    WHEN CC.OtherCareProfTeamLocalID IS NOT NULL THEN cc.UniqOtherCareProfTeamLocalID
    ELSE REF.UniqCareProfTeamLocalID END AS UniqOtherCareProfTeamLocalID
  ,CC.CareContDate
  ,CC.AttendStatus
 ...
 LEFT JOIN (SELECT * from $reference_data.datadictionarycodes WHERE ItemName = 'ATTENDANCE_STATUS') dss_corp_ATT
  ON CC.AttendStatus = dss_corp_ATT.PrimaryCode
 LEFT JOIN (SELECT * from $reference_data.datadictionarycodes WHERE ItemName = 'CONSULTATION_MECHANISM') dss_corp_CONS --CONSULTATION_MECHANISM???
 ON CC.ConsMechanismMH = dss_corp_CONS.PrimaryCode
 LEFT JOIN $db_source.MHS101Referral ref 
   ON cc.UniqServReqID = ref.UniqServReqID and cc.UniqMonthID = ref.UniqMonthID
 WHERE
 cc.UniqMonthID = '$month_id'
...
# DBTITLE 1,MHS29 Prep - National/CCG/Provider
 ...
 SELECT                CC.UniqCareContID
                       ...
                       ,CC.CareContCancelDate
                       ,CC.CareContDate
                       ,CASE WHEN CC.AttendStatus = 2 THEN 'Appointment cancelled by, or on behalf of the patient'
                             ...
                             ELSE 'N/A'
                             END AS DNA_Reason
                       ,CASE WHEN dss_corp_ATT.PrimaryCode IS NOT NULL THEN CC.AttendStatus
                      WHEN CC.AttendStatus IS NULL THEN 'UNKNOWN'
                      WHEN CC.AttendStatus IS NOT NULL AND dss_corp_ATT.PrimaryCode IS NULL THEN 'Invalid' END AS Attend_Code 
...
# DBTITLE 1,MHS29abc Prep - National and CCG
 ...
 SELECT                CC.UniqCareContID
                       ,SRV.ServTeamTypeMH as ServTeamTypeRefToMH
                       ,MPI.PatMRecInRP
                       ,CCG.NAME
                       ,CCG.IC_Rec_CCG
                       ,CC.AttendStatus
                       ,CASE WHEN CC.AttendStatus = 2 THEN 'Appointment cancelled by, or on behalf of the patient'
                             ...
                             ELSE 'N/A'
                             END AS DNA_Reason
                       ,CASE WHEN CC.ATT_PrimaryCode IS NOT NULL THEN CC.AttendStatus
                      WHEN CC.AttendStatus IS NULL THEN 'UNKNOWN'
                      WHEN CC.AttendStatus IS NOT NULL AND CC.ATT_PrimaryCode IS NULL THEN 'Invalid' END AS Attend_Code 
 ...
 LEFT JOIN            $db_source.MHS902ServiceTeamDetails AS SRV
                       ON CC.UniqOtherCareProfTeamLocalID = SRV.UniqCareProfTeamLocalID
                       AND SRV.UniqMonthID = '$month_id' 
 ...
 SELECT                CC.UniqCareContID
                  ...
                  -------------------------------------------------------------------------------------------------------------            
                      ...
                      ,CASE 
                       ...
                       ,CC.OrgIDProv
 FROM                  global_temp.CC001_Prep AS CC
 INNER JOIN            $db_source.MHS902ServiceTeamDetails AS SRV
                       ON CC.UniqOtherCareProfTeamLocalID = SRV.UniqCareProfTeamLocalID
                       AND SRV.UniqMonthID = '$month_id' 
 WHERE                     CC.UniqMonthID = '$month_id' 
                       AND (CareContDate >= '$rp_startdate' AND CareContDate <= '$rp_enddate')
...
# DBTITLE 1,MHS29d_prep
 ...
 SELECT               
                       ...
                       ,CC.CareContCancelDate
                       ,CC.CareContDate
                       ,CASE WHEN CC.AttendStatus IN ('2','3','4','7') THEN CC.ATT_Desc ELSE 'N/A' END AS DNA_Reason
                -------------------------------------------------------------------------------------------------------------      
                      ,CASE WHEN CC.ConsMechanismMH IS NULL THEN 'Missing'
                             WHEN CC.ConsMechanismMH NOT IN ('01','02','04','05','09','10','11','12','13','98') THEN 'Invalid'
                             ELSE CC.ConsMechanismMH
  ...
  WHERE      CC.UniqMonthID = '$month_id' 
            AND (CareContDate >= '$rp_startdate' AND CareContDate <= '$rp_enddate')
            AND SRV.ServTeamTypeMH in ('A05','A06','A08','A09','A12','A13','A16','C10');
...
# DBTITLE 1,MHS29f_prep
 ...
 SELECT                CC.UniqCareContID
                       ...
                       ,CASE WHEN CC.ConsMechanismMH IS NULL THEN 'Missing'
                             ...
                             ELSE 'Invalid'
                             END AS CMU 
                        ,CASE WHEN CC.AttendStatus = 5 THEN 5
                             WHEN CC.AttendStatus = 6 THEN 6
                             WHEN CC.AttendStatus = 2 THEN 2
                             WHEN CC.AttendStatus = 3 THEN 3
                             WHEN CC.AttendStatus = 4 THEN 4
                             WHEN CC.AttendStatus = 7 THEN 7
                             WHEN CC.AttendStatus is null THEN 'UNKNOWN'
                             ELSE 'Invalid'
                             END AS Attend_Code
                       ,CASE WHEN CC.AttendStatus = 5 THEN 'Attended on time or, if late, before the relevant professional was ready to see the patient'
                             WHEN CC.AttendStatus = 6 THEN 'Arrived late, after the relevant professional was ready to see the patient, but was seen'
...
# DBTITLE 1,MHS58 Prep
 ...
 WHERE        A.AttendStatus IN ('2', '3', '4', '7')
