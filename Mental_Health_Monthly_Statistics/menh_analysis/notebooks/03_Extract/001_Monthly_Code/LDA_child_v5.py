...
# COMMAND ----------
 ...
 from  $db_output.ServiceTeamType c 
...
# COMMAND ----------
 ...
 %sql
 --Individuals identified within table MHS609
  
 CREATE OR REPLACE TEMPORARY VIEW LD_609 AS
  
 select distinct Person_ID, '609' as table
 from $db_source.mhs609prescomp k 
 where k.FindSchemeInUse = '01' and (k.PresComp like 'F7%' or k.PresComp like 'F84%')
 and UniqMonthID = $month_id;
...
# COMMAND ----------
 ...
 SELECT  M.Person_ID,
         ...
         DD.DelayDischReason,
 Case when PVD.ProvDiag = 'Z755' or PRD.PrimDiag = 'Z755' or SED.SecDiag = 'Z755' or PC.PresComp = 'Z755' then 'Y'
 ...
 FROM    $db_source.MHS001MPI M 
         ...
         LEFT JOIN $db_source.MHS609PresComp as PC ON R.person_ID = PC.person_ID AND PC.UniqMonthID = '$month_id' AND PC.prescomp ='Z755' -- and SED.ic_use_submission_flag = 'y' -- needed  for testing in hue
         -------ADD IN MHS609 TABLE HERE-----------
         ...
         LEFT JOIN (
           select UniqMonthID, UniqHospProvSpellID, StartDateDelayDisch, EndDateDelayDisch, DelayDischReason from $db_source.MHS504DelayedDischarge WHERE UniqMonthID = '$month_id'
           union all 
           select UniqMonthID, UniqHospProvSpellID, StartDateClinReadyforDisch as StartDateDelayDisch, EndDateClinReadyforDisch as EndDateDelayDisch, ClinReadyforDischDelayReason as DelayDischReason from $db_source.MHS518ClinReadyforDischarge
           WHERE UniqMonthID = '$month_id'
         ) as DD ON H.UniqHospProvSpellID = DD.UniqHospProvSpellID AND DD.EndDateDelayDisch IS NULL-- and DD.ic_use_submission_flag = 'y' -- needed for testing in hue
         LEFT JOIN 
...
# COMMAND ----------
 ...
 CASE WHEN dd.code is not null then dd.code
      else '35' end as PrimaryMeasureNumber,
 CASE WHEN dd.description is not null then dd.description
      else 'Unknown' end as PrimaryMeasureSplit,
 '' as SecondaryMeasure,
 '' as SecondaryMeasureNumber,
 '' as SecondarySplit,
 '0' as RestraintsCountOfPeople,
 ...
 FROM 
  $db_output.LDA_Data_1 d
 LEFT JOIN $db_output.DelayedDischDim dd on d.DelayDischReason = dd.code and dd.key = 'reason' and '$month_id' >= dd.FirstMonth and (dd.LastMonth is null or '$month_id' <= dd.LastMonth)
...
# COMMAND ----------
 ...
 c1.FindSchemeInUse,
 c1.Procedure,
 CASE WHEN c1.Procedure = '1060751000000101' THEN "CommunityCTR"
      WHEN c1.Procedure = '1060761000000103' THEN "InpatientCTR"
      WHEN c1.Procedure = '1060741000000104' THEN "PostAdmissionCTR"
 ...
 get_provider_type(CASE
   ...
   and c1.Procedure in ('1060751000000101','1060761000000103','1060741000000104')
...
def add_zero_column(df: F.DataFrame, col:str) -> F.DataFrame:
  
  """
  
  Background
  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  This function is designed to stop code failing when a column doesn't exist in one month to another. 
  If the column does not exist - it will add a column with that name which is made up just of zero values.
  If the column does exist - nothing will be added to the dataframe
  
  Parameters
  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  df: DataFrame
       A dataframe to check the existence of a specific column
  col:str
       The column name
       
  Example Usage
  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  %run "/data_analysts_collaboration/CMH_FIBONACCI_Team(DQ)/MHSDS_DQ_Common_Functions/Admin/helpers"
   
  %run "/data_analysts_collaboration/CMH_FIBONACCI_Team(DQ)/MHSDS_DQ_Common_Functions/Policy_Areas/Acute_Crisis_Pathways/Emails/clinical_response_codes"
   
  cr_summary = get_crisis_pathway_clinical_response_codes_summary(1474)
  cr_summary_check = add_zero_column(cr_summary, 'InvalidCode')
 
  """
  
  list = df.columns
  name_list = spark.createDataFrame(list, T.StringType())
  
  name_check = name_list.filter(F.col('value') == col)
...
#Add zero column in case of not existing
piv_df_fill = add_zero_column(piv_df_fill, col="CommunityCTR")
piv_df_fill = add_zero_column(piv_df_fill, col="PostAdmissionCTR")
piv_df_fill = add_zero_column(piv_df_fill, col="InpatientCTR")
...
# COMMAND ----------
 ...
 %sql
 CREATE OR REPLACE GLOBAL TEMP VIEW lda_csv_lookup_tb_mn AS
 SELECT
 tb.Geography,
 tb.OrgCode,
 tb.OrgName,
 tb.TableNumber,
 tb.PrimaryMeasure,
 mn.PrimaryMeasureNumber,
 tb.PrimarySplit,
 tb.SecondaryMeasure,
 mn.SecondaryMeasureNumber,
 tb.SecondarySplit
 from global_temp.lda_csv_lookup_tb tb
 left join $db_output.lda_measure_number_ref mn
 on tb.TableNumber = mn.TableNumber
 and tb.PrimaryMeasure = mn.PrimaryMeasure
 and tb.PrimarySplit = mn.PrimarySplit
 and tb.SecondaryMeasure = mn.SecondaryMeasure
 and tb.SecondarySplit = mn.SecondarySplit
...
# COMMAND ----------
 ...
 SELECT
 COALESCE(l.PreviousMonthEnd, '$PreviousMonthEnd') as PreviousMonthEnd,
 COALESCE(l.PeriodStart, '$rp_startdate') as PeriodStart,
 COALESCE(l.PeriodEnd, '$rp_enddate') as PeriodEnd,
 tb.Geography,
 tb.OrgCode,
 tb.OrgName,
 tb.TableNumber,
 tb.PrimaryMeasure,
 tb.PrimaryMeasureNumber,
 tb.PrimarySplit,
 tb.SecondaryMeasure,
 tb.SecondaryMeasureNumber,
 tb.SecondarySplit,
 COALESCE(l.RestraintsCountOfPeople,0) as RestraintsCountOfPeople,
 COALESCE(l.RestraintsCountOfRestraints,0) as RestraintsCountOfRestraints,
 COALESCE(l.HospitalSpellsInCarePreviousMonth,0) as HospitalSpellsInCarePreviousMonth,
 COALESCE(l.HospitalSpellsAdmissionsInMonth,0) as HospitalSpellsAdmissionsInMonth,
 COALESCE(l.HospitalSpellsDischargedInMonth,0) as HospitalSpellsDischargedInMonth,
 COALESCE(l.HospitalSpellsAdmittedAndDischargedInMonth,0) as HospitalSpellsAdmittedAndDischargedInMonth,
 COALESCE(l.HospitalSpellsOpenAtEndOfMonth,0) as HospitalSpellsOpenAtEndOfMonth,
 COALESCE(l.WardStaysInCarePreviousMonth,0) as WardStaysInCarePreviousMonth,
 COALESCE(l.WardStaysAdmissionsInMonth, 0) as WardStaysAdmissionsInMonth,
 COALESCE(l.WardStaysDischargedInMonth,0) as WardStaysDischargedInMonth,
 COALESCE(l.WardStaysAdmittedAndDischargedInMonth,0) as WardStaysAdmittedAndDischargedInMonth,
 COALESCE(l.WardStaysOpenAtEndOfMonth,0) as WardStaysOpenAtEndOfMonth,
 COALESCE(l.OpenReferralsPreviousMonth,0) as OpenReferralsPreviousMonth,
 COALESCE(l.ReferralsStartingInTheMonth,0) as ReferralsStartingInTheMonth,
 COALESCE(l.ReferralsEndingInTheMonth,0) as ReferralsEndingInTheMonth,
 COALESCE(l.ReferralsStartingAndEndingInTheMonth,0) as ReferralsStartingAndEndingInTheMonth,
 COALESCE(l.ReferralsOpenAtEndOfMonth,0) as ReferralsOpenAtEndOfMonth,
  
 case 
 when tb.Tablenumber not in ('70','71','72','73','74','90') then null
 when tb.OrgCode = 'Invalid' then 'Invalid'
 when tb.OrgName like '%NHS%' then 'NHS'
 else 'Independent' end as NHS_NHD_SPLIT,
 case
 when tb.Tablenumber not in ('90', '100', '101') then null
 when tb.TableNumber = '100' then "Commissioning Region"
 when tb.TableNumber = '101' then 'ICB'
 else ORG_TYPE_CODE end as ORG_TYPE_CODE
  
  
 from global_temp.lda_csv_lookup_tb_mn tb
 left join $db_output.LDA_Counts l
 on tb.Geography = l.Geography
 and tb.OrgCode = l.OrgCode
 and tb.TableNumber = l.TableNumber
 and tb.PrimaryMeasure = l.PrimaryMeasure
 and tb.PrimaryMeasureNumber = l.PrimaryMeasureNumber
 and tb.PrimarySplit = l.PrimarySplit
 and tb.SecondaryMeasure = l.SecondaryMeasure
 and tb.SecondaryMeasureNumber = l.SecondaryMeasureNumber
 and tb.SecondarySplit = l.SecondarySplit
 and l.PRODUCT ='Monthly' 
 and l.PeriodEnd = '$rp_enddate'
 and l.SOURCE_DB = '$db_source'
  
 order by tb.TableNumber,tb.PrimaryMeasureNumber,tb.SecondaryMeasureNumber
