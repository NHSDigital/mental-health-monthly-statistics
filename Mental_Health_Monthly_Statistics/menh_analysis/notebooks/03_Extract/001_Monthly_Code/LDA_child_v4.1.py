...
# COMMAND ----------
 ...
 FROM	$db_source.MHS001MPI M 
         ...
         LEFT JOIN $db_source.MHS505RestrictiveIntervention_pre_v5 as RES ON RES.WardStayId = A.WardStayId AND RES.person_ID = A.person_ID 
...
# COMMAND ----------
 ...
 select P.*
        ...
        ELSE O5.NAME 
        END as OrgName
 From global_temp.LDA_Data_P P
 left join global_temp.RD_ORG_DAILY_LATEST_LDA O4 
       on O4.ORG_CODE = (case when P.CombinedCommissioner like 'YDD%' then P.CombinedCommissioner  
       else (left(P.CombinedCommissioner,3)) 
       end)                       
 left join global_temp.RD_ORG_DAILY_LATEST_LDA O5 
       on O5.ORG_CODE = left(P.CombinedCommissioner,3) ;
 ...
 select T.*,
       ...
       R.NAME AS ORG_NAME_R,
       case 
         when R.ORG_TYPE_CODE IS NULL then 'Invalid' 
         else R.ORG_TYPE_CODE   
         end 
       as ORG_TYPE_CODE
 from global_temp.LDA_Data_T T
...
# COMMAND ----------
 ...
 CASE
        WHEN DestOfDischHospProvSpell in ('19','29','54','65','66','85') then '1'
        WHEN DestOfDischHospProvSpell in ('30','48','49','50','51','52','53','84','87') then '2'
        WHEN DestOfDischHospProvSpell in ('37','38') then '3'
        ...
        END as PrimaryMeasureNumber,
 CASE
        WHEN DestOfDischHospProvSpell in ('19','29','54','65','66','85') then 'Community'
        WHEN DestOfDischHospProvSpell in ('30','48','49','50','51','52','53','84','87') then 'Hospital'
        WHEN DestOfDischHospProvSpell in ('37','38') then 'Penal Establishment / Court'
        WHEN DestOfDischHospProvSpell in ('88') then 'Non-NHS Hospice'
