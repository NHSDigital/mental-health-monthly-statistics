...
# DBTITLE 1,Clean unformatted output table (in case there already is left-over data for this month/status in the table)
 ...
 DELETE FROM $db_output.CYP_2nd_contact_unformatted
 WHERE REPORTING_PERIOD_END = '$rp_enddate'
 AND STATUS = '$status'
 AND SOURCE_DB = '$db_source'
 ;
 VACUUM $db_output.CYP_2nd_contact_unformatted RETAIN 8 HOURS

# COMMAND ----------

# DBTITLE 1,Aggregate CYP 2nd contact
dbutils.notebook.run("../02_Aggregate/3.CYP_2nd_contact_agg", 0, params)
