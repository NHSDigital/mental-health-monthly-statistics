...
# DBTITLE 1,MHS32a/b National and CCG
 ...
 CREATE OR REPLACE GLOBAL TEMPORARY VIEW MH32_prep AS
   SELECT   REF.UniqServReqID 
            ...
            ,MPI.IC_Rec_CCG
            ,MPI.NAME
            ,CASE WHEN REF.SourceOfReferralMH IN ('A1','A2','A3') THEN 'A' 
                  WHEN REF.SourceOfReferralMH IN ('B1','B2') THEN 'B' 
                  WHEN REF.SourceOfReferralMH IN ('C1','C2') THEN 'C' 
                  WHEN REF.SourceOfReferralMH = 'D1' THEN 'D' 
                  ...
                  WHEN REF.SourceOfReferralMH IN ('J1','J2','J3','J4') THEN 'J' 
                  WHEN REF.SourceOfReferralMH IN ('K1','K2','K3','K4','K5') THEN 'K' 
                  WHEN REF.SourceOfReferralMH IN ('L1','L2') THEN 'L' 
                  WHEN REF.SourceOfReferralMH IN ('M1','M2','M3','M4','M5','M6','M7') THEN 'M' 
            ...
            ,CASE WHEN REF.SourceOfReferralMH IN ('A1','A2','A3') THEN 'Primary Health Care' 
                 WHEN REF.SourceOfReferralMH IN ('B1','B2') THEN 'Self Referral' 
                 WHEN REF.SourceOfReferralMH IN ('C1','C2') THEN 'Local Authority Services' 
                 WHEN REF.SourceOfReferralMH = 'D1' THEN 'Employer' 
                 ...
                 WHEN REF.SourceOfReferralMH IN ('J1','J2','J3','J4') THEN 'Internal referrals  from Community Mental Health Team (within own NHS Trust)' 
                 WHEN REF.SourceOfReferralMH IN ('K1','K2','K3','K4','K5') THEN 'Internal referrals from Inpatient Service (within own NHS Trust)' 
                 WHEN REF.SourceOfReferralMH IN ('L1','L2') THEN 'Transfer by graduation (within own NHS Trust)' 
                 WHEN REF.SourceOfReferralMH IN ('M1','M2','M3','M4','M5','M6','M7') THEN 'Other' 
...
# DBTITLE 1,MHS61b National Prep
 ...
 SELECT a.uniqcarecontid
        ,a.uniqservreqid
        ,a.carecontdate
        ,row_number() over (partition by a.uniqservreqid order by a.carecontdate asc, a.uniqcarecontid asc) as rn
 ...
 group by a.uniqservreqid, a.carecontdate, a.uniqcarecontid;
...
# DBTITLE 1,MHS61b CCG Prep
 ...
 SELECT a.uniqcarecontid, ccg.ic_rec_ccg, ccg.name, b.consmediumused
 ...
 and a.rn = 1
 GROUP BY ccg.ic_rec_ccg, ccg.name, a.uniqcarecontid, b.consmediumused;
...
# DBTITLE 1,MHS61b Prov Prep
 ...
 SELECT a.uniqcarecontid, b.consmediumused, b.orgidprov
 FROM global_temp.MHS61b as a
 INNER JOIN $db_source.mhs201carecontact as b
 ...
 GROUP BY a.uniqcarecontid, b.consmediumused, b.orgidprov;
 ...
 CREATE OR REPLACE GLOBAL TEMPORARY VIEW MHS68_prep AS
   ...
   INNER JOIN $db_source.MHS606CodedScoreAssessmentRefer	AS CSA
        ...
        WHERE CSA.UniqMonthID = '$month_id' 
 		     ...
 		     AND CSA.AssToolCompDate between '$rp_startdate' and '$rp_enddate'
