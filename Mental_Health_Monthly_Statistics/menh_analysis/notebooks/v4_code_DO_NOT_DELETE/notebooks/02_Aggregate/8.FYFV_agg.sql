...
INSERT INTO $db_output.FYFV_unformatted
--CREATE OR REPLACE GLOBAL TEMPORARY VIEW CYP_2nd_contact_unformatted AS
SELECT '$rp_startdate_m1' AS REPORTING_PERIOD_START,
       '$rp_enddate' AS REPORTING_PERIOD_END,
       '$status' AS STATUS,
       'England' AS BREAKDOWN,
...
LEFT JOIN $db_output.RD_CCG_LATEST DFC_CCG 
       ON w.OrgIDComm = DFC_CCG.ORG_CODE
LEFT JOIN $db_output.RD_CCG_LATEST AS CCG_REF
       ON ccg.IC_Rec_CCG  = CCG_REF.ORG_CODE
GROUP BY CASE
         WHEN w.OrgIDProv = 'DFC' THEN COALESCE(DFC_CCG.ORG_CODE, 'UNKNOWN')
...
LEFT JOIN $db_output.STP_Region_mapping_post_2018 
        AS stp 
...
FROM 
(SELECT DISTINCT Region_code,Region_description  FROM $db_output.STP_Region_mapping_post_2018) 
    AS a
...
from 
(SELECT DISTINCT STP_code,STP_description  FROM $db_output.STP_Region_mapping_post_2018) 
    AS a
