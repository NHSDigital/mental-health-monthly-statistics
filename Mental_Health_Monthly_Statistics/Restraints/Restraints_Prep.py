...
# DBTITLE 1,Region Ref Data Final 
 ...
 SELECT 
 A.ORG_CODE as STP_CODE, /********* STP  to ICB name change needed here? ***************/
 A.NAME as STP_NAME, /********* STP  to ICB name change needed here? ***************/
...
# DBTITLE 1,Restrictive Interventions Table Prep
 ...
 CASE WHEN StartDateRestrictiveInt < '$rp_startdate' THEN '$rp_startdate' ELSE StartDateRestrictiveInt END AS Der_RestIntStartDate,
 CASE WHEN EndDateRestrictiveInt is null then date_add('$rp_enddate', 1) ELSE EndDateRestrictiveInt END AS Der_RestIntEndDate,
 CASE WHEN starttimerestrictiveint is null then to_timestamp('1970-01-01') else starttimerestrictiveint end as Der_RestIntStartTime, 
 CASE WHEN endtimerestrictiveint is null then to_timestamp('1970-01-01') else endtimerestrictiveint end as Der_RestIntEndTime,
 case 
 ...
 and ((a.Startdaterestrictiveint BETWEEN '$rp_startdate' AND '$rp_enddate') 
                             or (a.Enddaterestrictiveint BETWEEN '$rp_startdate' AND '$rp_enddate')
                             or (a.Enddaterestrictiveint is null))
...
# DBTITLE 1,Ward Stay Table Prep
 ...
 select
 a.person_id, a.orgidprov, a.uniqmonthid, a.uniqsubmissionid, a.uniqwardstayid, a.siteidoftreat, a.mhs502uniqid, a.UniqHospProvSpellNum, a.startdatewardstay, a.enddatewardstay, a.inacttimews, 
...
# DBTITLE 1,MPI Table Prep
 ...
 select
 a.person_id, a.orgidprov, a.uniqmonthid, a.uniqsubmissionid, a.OrgIDCCGRes, a.AgeRepPeriodEnd, a.NHSDEthnicity, a.Gender, a.RecordNumber, a.PatMRecInRP, -- a.OrgIDCCGRes needs updating gvf
...
# COMMAND ----------
 ...
 %sql --a.OrgIDCCGRes needs updating gvf
...
# DBTITLE 1,Hospital Spell Table Prep
 ...
 select a.person_id, a.orgidprov, a.uniqmonthid, a.uniqsubmissionid, a.uniqservreqid, a.uniqhospprovspellnum, a.DischDateHospProvSpell 
...
# DBTITLE 1,Create Initial Prep Table - Part1
 ...
 r.restrictiveinttype,
 ws.SpecialisedMHServiceCode,
 ws.specialised_service,
 r.startdaterestrictiveint, 
 r.enddaterestrictiveint,
 r.starttimerestrictiveint,
 r.endtimerestrictiveint,
 r.Der_RestIntStartDate,
 r.Der_RestIntEndDate,
 r.pre_flag,
 ...
 r.mhs505uniqid, 
 ws.bed_days,
 ROW_NUMBER() OVER (PARTITION BY ws.uniqwardstayid ORDER BY r.MHS505UniqID) AS Der_RecordNumber
 from $db_output.RI_WARDSTAY ws
 left join $db_output.RI_RESTRAINTS r on ws.uniqwardstayid = r.uniqwardstayid
 left join $db_output.RI_MPI_FINAL m on ws.person_id = m.person_id and m.uniqmonthid = ws.uniqmonthid 
 left join $db_output.RI_HEADER h on ws.uniqmonthid = h.uniqmonthid
 left join menh_publications.RestrictiveIntTypeDim_Extended as int on r.restrictiveinttype = int.key
 ...
 left join $db_output.RI_STP_MAPPING stp on m.OrgIDCCGRes = stp.CCG_CODE /*************************** OrgIDCCGRes needs updating *****************/
...
# DBTITLE 1,Create Initial Prep Table - Part2
 ...
 ri.startdaterestrictiveint, 
 ri.enddaterestrictiveint,
 ri.starttimerestrictiveint,
 ri.endtimerestrictiveint,
 ri.Der_RestIntStartDate,
 ri.Der_RestIntEndDate,
 ri.pre_flag,
 case
 when Der_RestIntStartDate = Der_RestIntEndDate and starttimerestrictiveint = endtimerestrictiveint then 'N'
 when Der_RestIntStartDate = Der_RestIntEndDate and starttimerestrictiveint > endtimerestrictiveint then 'N'
