...
# COMMAND ----------
 ...
 percentile(minutes_of_restraint, 0.5) as median_min_of_restraint
 from $db_output.RI_FINAL
...
# COMMAND ----------
 ...
 %sql
 CREATE OR REPLACE GLOBAL TEMP VIEW total_breakdowns_active_time_test_type AS 
 select 
 coalesce(region_code, 'England') as region_code, 
 coalesce(region_name, 'England') as region_name,  
 coalesce(orgidprov, 'England') as orgidprov, 
 ...
 'Total' as variabletype,
 'Total' as variable,
 sum(days_of_restraint)/count(distinct UniqRestrictiveIntIncID, UniqRestrictiveIntTypeID) as avg_days_of_active_restraint,
 percentile(days_of_restraint, 0.5) as median_days_of_active_restraint
 from $db_output.RI_FINAL
 where person_id is not null and restrictiveintname in ("Seclusion", "Segregation") and RI_RecordNumber_Type = 1 and enddaterestrictiveintType is null and active_avg_min_flag_type = 'Y'
 group by grouping sets (
 (region_code, region_name),
 (region_code, region_name, orgidprov, orgidname),
 (region_code, region_name, specialised_service),
...
# COMMAND ----------
 ...
 percentile(minutes_of_incident, 0.5) as median_min_of_incident
 from $db_output.RI_FINAL
...
# COMMAND ----------
 ...
 a.median_min_of_restraint,
 coalesce(b.avg_min_of_incident, 0) as avg_min_of_incident,
 coalesce(b.max_min_of_incident, 0) as max_min_of_incident,
 coalesce(b.median_min_of_incident, 0) as median_min_of_incident,
 coalesce(c.avg_days_of_active_restraint, 0) as avg_days_of_active_restraint,
 coalesce(c.median_days_of_active_restraint, 0) as median_days_of_active_restraint
 ...
 LEFT JOIN global_temp.total_breakdowns_active_time_test_type c
 ON 
 a.region_code = c.region_code
 and a.region_name = c.region_name
 and a.orgidprov = c.orgidprov
 and a.orgidname = c.orgidname
 and a.specialised_service = c.specialised_service
 and a.restrictiveintcode = c.restrictiveintcode
 and a.restrictiveintname = c.restrictiveintname
 and a.patient_injury = c.patient_injury
 and a.variabletype = c.variabletype
...
# COMMAND ----------
 ...
 and hb.age_group = 'England'--Updated for PBI calculation
 and hb.gendercode = 'England'--Updated for PBI calculation
 left join $db_output.dash_ward_bed_days_pub_csv wb
 on a.region_code = wb.region_code
 and a.region_name = wb.region_name
 and a.orgidprov = wb.orgidprov
 ...
 and wb.age_group = 'England'--Updated for PBI calculation
 and wb.gendercode = 'England'--Updated for PBI calculation
 order by a.orgidprov
...
# COMMAND ----------
 ...
 COALESCE(b.median_min_of_restraint, 0) as median_min_of_restraint,
 COALESCE(b.avg_min_of_incident, 0) as avg_min_of_incident,
 COALESCE(b.max_min_of_incident, 0) as max_min_of_incident,
 COALESCE(b.median_min_of_incident, 0) as median_min_of_incident,
 coalesce(b.avg_days_of_active_restraint, 0) as avg_days_of_active_restraint,
 coalesce(b.median_days_of_active_restraint, 0) as median_days_of_active_restraint
...
# COMMAND ----------
 ...
 ,median_min_of_incident
 ,avg_days_of_active_restraint
 ,median_days_of_active_restraint) T1
