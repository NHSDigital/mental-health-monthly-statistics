...
LEFT JOIN 
    ...
    MAX(CASE WHEN ServDischDate IS NULL AND ReportingPeriodEndDate = '$rp_enddate' THEN 1
             WHEN DATEDIFF('$rp_enddate', EndDate) <= 5 AND ReportingPeriodEndDate = '$rp_enddate' THEN 1 ---added logic so that if enddate is 5 days before RP End it is still classed as open
...
WHERE 
    ...
    AND (s.Ref_Group IN 
        ...
        'Psychological therapies',
        'Psychosocial interventions')
        OR (s.Ref_Group = 'Assessments' AND (s.SNOMED_code = '1085671000000109' or s.SNOMED_code = '1914891000000100'))) 
