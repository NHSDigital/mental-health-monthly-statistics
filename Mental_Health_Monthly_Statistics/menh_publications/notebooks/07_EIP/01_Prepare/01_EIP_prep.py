...
# COMMAND ----------
 ...
 DROP TABLE IF EXISTS $db_output.MHS201CareContact_der;
 CREATE TABLE IF NOT EXISTS $db_output.MHS201CareContact_der AS
 ...
 left join $db_source.MHS101Referral ref on c.uniqmonthid = ref.uniqmonthid and c.uniqservreqid = ref.uniqservreqid
...
# DBTITLE 1,earliest_care_contact_dates_by_service_request_id
 ...
 CREATE OR REPLACE GLOBAL TEMP VIEW earliest_care_contact_dates_by_service_request_id AS 
   ...
   SELECT UniqServReqID, MIN(CareContDate) AS CareContDate
   FROM
   (          
    SELECT a.UniqServReqID, CareContDate
 --       FROM $db_source.MHS201CareContact AS a
          FROM $db_output.MHS201CareContact_der AS a
   ...
   LEFT JOIN $db_output.ServiceTeamType AS x
             ON a.UniqCareProfTeamLocalID_der = x.UniqCareProfTeamID
             AND a.UniqServReqID = x.UniqServReqID
             AND x.UniqMonthID <= $month_id
             AND a.uniqmonthid <= '1488'
   LEFT JOIN $db_source.MHS902ServiceTeamDetails srv
             ON a.UniqCareProfTeamLocalID_der = srv.UniqCareProfTeamLocalID
             and a.uniqmonthid = srv.uniqmonthid
             and a.uniqmonthid > '1488'
       WHERE a.AttendStatus IN ('5', '6')
             AND (x.ServTeamTypeRefToMH = 'A14' OR srv.ServTeamTypeMH = 'A14')
