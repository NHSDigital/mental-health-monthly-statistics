...
# COMMAND ----------
 ...
 --    'NULL' AS RepApptOfferDate,
  --   'NULL' AS RepApptBookDate,
     ...
     CASE WHEN uniqmonthid > '1488' THEN CONCAT(i.OrgIDProv,i.OtherCareProfTeamLocalID)
...
# DBTITLE 1,Contacts data
 ...
 SELECT                ca.RecordNumber,
                       ...
                       ca.Procedure as Procedure, --gets first snomed code in list where IndActProcedure contains a ":"
                       CASE WHEN position(':' in ca.Procedure) > 0 THEN LEFT(ca.Procedure, position (':' in ca.Procedure)-1) 
                            ELSE ca.Procedure
 ...
 SELECT 	              i.RecordNumber,
                       ...
                       i.IndActProcedure as Procedure,  /*** Procedure renamed to IndActProcedure ***/
                       --gets first snomed code in list where IndActProcedure contains a ":"
                       CASE WHEN position(':' in i.IndActProcedure) > 0 THEN LEFT(i.IndActProcedure, position (':' in i.IndActProcedure)-1) 
                            ELSE i.IndActProcedure /*** Procedure 4 occurrences renamed to IndActProcedure ***/
                            END AS Der_SNoMEDProcCode,
                       NULL AS Observation
 FROM                  $db_source.mhs204indirectactivity i
...
# DBTITLE 1,Listing relevant snomed codes for EIP
 ...
 SELECT              r.Person_ID,
                     ...
                     --duplicated these codes to pick up those recorded as OBS
                             OR c.Observation =o.conceptid)
                   THEN 'NICE concordant' 
 	WHEN COALESCE(c.Der_SNoMEDProcCode,c.Observation) IS NOT NULL THEN 'Other' 
 ...
 INNER JOIN        global_temp.EIP_Interventions_DMS c ON r.recordnumber = c.recordnumber
  ...
  left join $db_output.EIP_Nice_Snomed n on n.conceptid = c.Der_SNoMEDProcCode
   left join $db_output.EIP_Nice_Snomed o on o.conceptid = c.Observation
