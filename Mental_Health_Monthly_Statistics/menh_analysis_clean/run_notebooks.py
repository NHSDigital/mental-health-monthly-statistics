...
# needs to use source_db for both but the parameter is fed in from run_tests as the original source_db :o(

# get the original parameter value (will work in all situations)
try:
  db_source = dbutils.widgets.get("source_db")
except:
  print('source_db is not defined')

# get the new parameter value (will only work in direct run, and will overwrite value for source_db)
try:
  db_source = dbutils.widgets.get("source_db")
except:
  print('source_db is not defined')
...
# dbutils.widgets.text("source_db", "", "Input database")
# db_source = dbutils.widgets.get("source_db")
# assert db_source
dbutils.widgets.text("source_db","source_db","Source Ref Database")
