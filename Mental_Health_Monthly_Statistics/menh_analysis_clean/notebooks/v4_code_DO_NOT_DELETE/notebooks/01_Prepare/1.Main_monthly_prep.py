...
# DBTITLE 1,MH01 Intermediate Provider
 ...
 SELECT				  MPI.Person_ID
                       ...
                       ,CASE WHEN AgeRepPeriodEnd BETWEEN 0 AND 18 THEN '00-18'
 							...
 							WHEN AgeRepPeriodEnd >=65 THEN '65-120' 
 							END AS AGE_GROUP
 FROM				  $db_source.MHS001MPI AS MPI -- table in generic prep
 INNER JOIN 			  $db_output.MHS101Referral_open_end_rp AS REF  -- table in generic prep
 					  ON MPI.Person_ID = REF.Person_ID
...
# DBTITLE 1,MHS29 Prep - National/CCG/Provider
 ...
 SELECT                CC.UniqCareContID
                       ...
                       ,CASE WHEN CC.AttendOrDNACode = 2 THEN 'Appointment cancelled by, or on behalf of the patient'
                             ...
                             ELSE 'N/A'
                             END AS DNA_Reason
                       ,CASE WHEN CC.ConsMediumUsed IS NULL THEN 'Missing'
                             WHEN CC.ConsMediumUsed NOT IN ('01','02','03','04','05','06','98') THEN 'Invalid'
                             ELSE CC.ConsMediumUsed
                             END AS ConsMedUsed
                       ,CASE WHEN CC.ConsMediumUsed IS NULL THEN 'Missing'
                             WHEN CC.ConsMediumUsed = '01' THEN 'Face to face communication'
                             WHEN CC.ConsMediumUsed = '02' THEN 'Telephone'
                             WHEN CC.ConsMediumUsed = '03' THEN 'Telemedicine web camera'
                             WHEN CC.ConsMediumUsed = '04' THEN 'Talk type for a person unable to speak'
                             WHEN CC.ConsMediumUsed = '05' THEN 'Email'
                             WHEN CC.ConsMediumUsed = '06' THEN 'Short Message Service (SMS) - Text Messaging'
                             WHEN CC.ConsMediumUsed = '98' THEN 'Other'
                             ELSE 'Invalid'
                             END AS CMU                      
