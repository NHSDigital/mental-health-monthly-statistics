...
# DBTITLE 1,MHS13 - Provider
 ...
 INNER JOIN  global_temp.accomodation_latest AS ACC 
             ON REF.Person_ID = ACC.Person_ID AND MPI.OrgIDProv = ACC.OrgIDProv --AND ACC.IC_Use_Submission_Flag = 'Y'  -- needed in Hue 
 WHERE       MPI.UniqMonthID = '$month_id'
 --          AND MPI.IC_Use_Submission_Flag = 'Y'
...
# DBTITLE 1,MHS23b
 ...
 INSERT INTO $db_output.Main_monthly_unformatted
   ...
   INNER JOIN $db_source.MHS102ServiceTypeReferredTo AS SRV	
        ...
        WHERE SRV.ServTeamTypeRefToMH IN ('A02', 'A03')
              AND SRV.Uniqmonthid = '$month_id'
     GROUP BY REF.OrgIDProv;
