...
-- DBTITLE 1,Primary Reason for Referral (Mental Health)
 ...
 AND IFNULL(mhs102.ReferRejectReason, 'xx') <> '02'
...
-- DBTITLE 1,Service Or Team Type Referred To (Mental Health)
 ...
 SELECT
   ...
   SUM(CASE
         ...
         ELSE 0
       END) AS Missing
 FROM $dbm.mhs102servicetypereferredto as ref
 LEFT JOIN $db_output.validcodes as vc
 ON vc.tablename = 'MHS102ServiceTypeReferredTo' and vc.field = 'ServTeamTypeRefToMH' and vc.Measure = 'MHS-DQM18' and vc.type = 'VALID' and upper(ref.ServTeamTypeRefToMH) = vc.ValidValue and $month_id >= vc.FirstMonth and (vc.LastMonth is null or $month_id <= vc.LastMonth)
 WHERE UniqMonthID = $month_id
 GROUP BY OrgIDProv;
...
-- DBTITLE 1,Hospital Bed Type (Mental Health) (Ward stays started on or after 1st April 2017)
 ...
 LEFT JOIN $db_output.validcodes as vc
 ON vc.tablename = 'MHS502WardStay' and vc.field = 'HospitalBedTypeMH' and vc.Measure = 'MHS-DQM31' and vc.type = 'VALID' and ref.HospitalBedTypeMH = vc.ValidValue and $month_id >= vc.FirstMonth and (vc.LastMonth is null or $month_id <= vc.LastMonth)
 WHERE UniqMonthID = $month_id
