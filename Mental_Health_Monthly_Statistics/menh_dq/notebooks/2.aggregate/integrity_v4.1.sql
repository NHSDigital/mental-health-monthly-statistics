...
(
  SELECT
    ...
    CASE
       -- WHEN e.CAMHSTier IN ('1','2','3','4','9') THEN 1
        WHEN e.ServTeamTypeRefToMH IN ('C05','C06','C07') THEN 1
  ...
  FROM $dbm.mhs101referral r
    ...
    LEFT OUTER JOIN $dbm.mhs102servicetypereferredto e ON r.UniqServReqID = e.UniqServReqID AND e.UniqMonthID = '$month_id'
  ...
  FROM $dbm.mhs101referral r
    ...
    INNER JOIN $dbm.mhs502wardstay w ON w.UniqHospProvSpellNum = h.UniqHospProvSpellNum AND w.StartDateWardStay = h.StartDateHospProvSpell AND w.UniqMonthID = '$month_id'
    INNER JOIN $dbm.mhs503assignedcareprof a ON a.UniqHospProvSpellNum = h.UniqHospProvSpellNum AND a.StartDateAssCareProf = h.StartDateHospProvSpell AND a.UniqMonthID = '$month_id'
  WHERE r.UniqMonthID = '$month_id'
    AND r.ReferralRequestReceivedDate >= '$rp_startdate'
    AND r.ReferralRequestReceivedDate <= '$rp_enddate'
    AND r.AgeServReferRecDate  < 19
