...
-- only cells which have been added for CYP_ED_WT have been noted here

- RD_CCG_LATEST - TABLE
- CCG_PRAC - view
- CCG_prep_3months [GBT changed from ed_CCG_prep as the tables are the same] - view
- CCG_PREP - view
- CCG - TABLE
...
- MHS001_CCG_LATEST - [GBT changed from ED_CCG_LATEST] - TABLE
...
--  If it's decided we need 'Operational' dates as preference then add desc                         
        ...
        FROM $database.ODSAPIRoleDetails rd
        
        LEFT JOIN $database.ODSAPIOrganisationDetails od
        ON rd.OrganisationID = od.OrganisationID and rd.DateType = od.DateType
        
        LEFT JOIN $database.ODSAPISuccessorDetails as odssd
...
# DBTITLE 1,Provider_list (rolling quarter)
%sql
...
REL_CLOSE_DATE
FROM 
$database.ORG_RELATIONSHIP_DAILY
WHERE
...
SELECT
  ...
  A.ReferringCareProfessionalStaffGroup,
  A.RowNumber,
...
FROM  $database.ONS_CHD_GEO_LISTINGS as a
      INNER JOIN $database.ONS_CHD_GEO_LISTINGS as b
...
select PrimaryCode, Description from $database.DataDictionaryCodes
