...
try:
  for run in jobs:
    ...
    if(params['automatic_run'] and run == 'Performance'):
      ...
      params['status'] = run
    # COMMON PARAMETER ADHOC CALCULATIONS THAT APPLIES BOTH PERFORMANCE AND PROVIDISIONAL IN CUSTOM/AUTOMATIC RUN
    ...
    if not product:
      ...
      rows = metadata_df.select('notebook_path')\
                        .where((col('module') == lit('run_notebooks')) & (col("product") == lit(product)))\
                        .sort(col("seq"))\
                        .collect()
      print(rows)
      if not rows:
...
#     Hardcoded mapping fixed by changes to creation of $db_output.RD_CCG_LATEST within notebooks/00_common_objects/02_load_common_ref_data
  ...
  # Using different notebooks rather than single note book with multiple if conditions on statuses and quarterly flag to ease the code writing and spotting based on statuses
