...
# DBTITLE 1,CODE 24 - Inappropriate OAPS Active at end of Period
# Prep tables build
...
# COMMAND ----------
 ...
 SELECT  '24' AS metric,
         'Number of Inappropriate OAPs placements (not bed-nights) active at the end of the period' AS metric_description

# COMMAND ----------

# DBTITLE 1,Remove Code 24 results from output
 %sql
...
# DBTITLE 1,England Month
 ...
 FROM (    SELECT
             ...
             COUNT(DISTINCT CASE WHEN ReasonOAT = 10 and (NewServDischDate is null or NewServDischDate>'$rp_enddate') THEN UniqServReqID ELSE null END) metric_value
...
# DBTITLE 1,Sending Provider, Month
 ...
 FROM (    SELECT
           ...
           LEFT JOIN $db_output.OAPs_ORG_DAILY as Org
              ON LEFT(a.OrgIDSubmitting,3) = Org.ORG_CODE
           --WHERE uniqmonthid = $end_month_id
