...
# DBTITLE 1,OAPS03 - OAPS Active at end of Period
# Prep tables build
...
# COMMAND ----------
 ...
 -- SELECT  'OAP03' AS metric,
 --         'Number of OAPs (not bed-nights) active at the end of the period' AS metric_description
 -- UNION ALL
 SELECT  'OAP03' AS metric,
         'Number of Inappropriate OAPs (not bed-nights) active at the end of the period' AS metric_description
...
counts_metadata = {##'OAP03':  {'type'  : 'count',
  ...
  for rp_startdate in rp_startdates:
    ...
    if reasonoat != '' and activeend != '':
      spark.sql(f"""CREATE TABLE IF NOT EXISTS {db_output}.DATA_{metric}_{rp_startdate} USING DELTA AS
                    ...
                    FROM {db_output}.OAPS_Month as a
