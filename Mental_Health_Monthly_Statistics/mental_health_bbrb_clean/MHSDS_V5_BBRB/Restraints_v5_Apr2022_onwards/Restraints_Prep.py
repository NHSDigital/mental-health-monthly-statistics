...
# DBTITLE 1,Region Ref Data Part1
 ...
 CREATE TABLE $db_output.RESTR_ORG_DAILY_A USING DELTA AS 
...
# DBTITLE 1,2. Restrictive Interventions Table Prep
 ...
 a.recordnumber,
 case when startdaterestrictiveintinc < '$rp_startdate' 
...
# DBTITLE 1,3. Restrictive Intervention Type Table Prep
 ...
 case when startdaterestrictiveintType < '$rp_startdate' 
...
# DBTITLE 1,4. Ward Stay Table Prep
 ...
 inner join $db_output.RI_SUBS b on a.uniqsubmissionid = b.uniqsubmissionid and a.uniqmonthid = b.uniqmonthid and a.orgidprov = b.orgidprovider
 left join menh_dq.dq_smh_service_category_code sc on sc.ServiceCategoryCode = a.SpecialisedMHServiceCode and (sc.LastMonth >='$month_id' or sc.LastMonth is NULL)
 where a.uniqmonthid = '$month_id'
...
# DBTITLE 1,5. MPI Table Prep
 ...
 select
 a.person_id, a.orgidprov, a.uniqmonthid, a.uniqsubmissionid, a.AgeRepPeriodEnd, a.NHSDEthnicity, ---will eventually need to be changed to Ethnicity2021 when cencus 2021 data released
...
# DBTITLE 1,Create Initial Prep Table - Part1
 ...
 cast(unix_timestamp(cast(concat(cast(Der_WrdStartDate as string), right(cast(Der_WrdStartTime as string), 9)) as timestamp)) as timestamp) as startdatetimewardstay,
 ...
 cast(unix_timestamp(cast(concat(cast(Der_RestIntIncStartDate as string), right(cast(Der_RestIntIncStartTime as string), 9)) as timestamp)) as timestamp) as startdatetimerestrictiveintinc,
 cast(unix_timestamp(cast(concat(cast(Der_RestIntIncEndDate as string), right(cast(Der_RestIntIncEndTime as string), 9)) as timestamp)) as timestamp) as enddatetimerestrictiveintinc,
 ...
 left join $db_output.RI_RESTRAINT_TYPE rt on rs.UniqRestrictiveIntIncID = rt.UniqRestrictiveIntIncID and rs.person_id = rt.person_id and rs.uniqhospprovspellid = rt.uniqhospprovspellid
...
# COMMAND ----------
 ...
 %sql
 select count(distinct UniqRestrictiveIntIncID, UniqRestrictiveIntTypeID) from $db_output.RI_FINAL
...
# DBTITLE 1,Bed day calculation wardstay related bkdowns (Sp Service, provider site)
 ...
 CREATE TABLE $db_output.WS_bed_days_pub_csv AS
 select 
 coalesce(age_group, 'NULL') as bd_age_group, 
 coalesce(gendercode, 'NULL') as bd_gendercode, 
 coalesce(gendername, 'NULL') as bd_gendername, 
