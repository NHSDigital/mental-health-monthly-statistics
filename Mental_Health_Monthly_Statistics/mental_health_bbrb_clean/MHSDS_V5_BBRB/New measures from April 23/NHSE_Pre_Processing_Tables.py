...
# DBTITLE 1,Referrals
 ...
 r.InactTimeRef,
 m.MHS001UniqID,
 CASE WHEN m.UniqMonthID <= 1467 then m.OrgIDCCGRes 
 ...
 m.NHSDEthnicity,
 m.Gender,
 CASE WHEN m.GenderIDCode IN ('1','2','3','4','X','Z') THEN m.GenderIDCode ELSE m.Gender END AS Gender2021, 
 m.MaritalStatus,
 m.PersDeathDate,
 m.AgeDeath,
...
# DBTITLE 1,Pre_Proc_Assessments PREP
 ...
 INNER JOIN $db_source.mhs101referral r ON r.RecordNumber = c.RecordNumber 
                                        AND c.AssToolCompDate BETWEEN r.ReferralRequestReceivedDate AND COALESCE(r.ServDischDate,h.ReportingPeriodEndDate) 
...
# DBTITLE 1,Pre_Proc_Assessments Stage 2 - Adding in MH_Assessments Reference data
 ...
 SELECT 
     ...
     r.Rater,
 	CASE 
 		WHEN CAST(a.PersScore as float) BETWEEN r.Lower_Range AND r.Upper_Range THEN 'Y' 
 		ELSE NULL 
