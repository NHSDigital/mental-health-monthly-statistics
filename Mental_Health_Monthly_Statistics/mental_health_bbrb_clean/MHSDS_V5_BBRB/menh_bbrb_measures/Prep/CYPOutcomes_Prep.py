...
# DBTITLE 1,CYP Outcomes - Closed Referrals
 ...
 AND r.UniqMonthID BETWEEN $start_month_id AND $end_month_id ----Jan 2020 to Dec 2021
 AND r.ReferralRequestReceivedDate >= '2016-01-01' 
 AND r.ReferRejectionDate IS NULL 
 -- AND r.ServDischDate IS NOT NULL --- discharged referrals only, not counting inactive referrals ----Removed from TB code 10/05/22
 AND (r.ServDischDate BETWEEN r.ReportingPeriodStartDate AND r.ReportingPeriodEndDate) ---discharged in that month ---Added to TB code 10/05/22
 ...
 AND (r.ServTeamTypeRefToMH NOT IN ('B02','E01','E02','E03','E04','A14') OR r.ServTeamTypeRefToMH IS NULL)--- exclude specialist teams *but include those where team type is null ---Added after discussion with TB 25/04/22
...
# DBTITLE 1,Get contacts for Closed CYP Referrals
 ...
 SELECT 
 	...
 	,MAX(CASE WHEN a.Der_ContactOrder = 1 THEN a.Der_ContactDate ELSE NULL END) AS Contact1 ---MAX() needed here? 
 	,MAX(CASE WHEN a.Der_ContactOrder = 2 THEN a.Der_ContactDate ELSE NULL END) AS Contact2 ---MAX() needed here?
 ...
 -- WHERE r.RN = 1 ---Removed from TB code 10/05/22
...
# DBTITLE 1,Get all assessments related to closed referrals table
 ...
 -- 	,'HoNOS-CA (Child and Adolescent)' NHS D Ref data is more specific AT adding in below
...
# DBTITLE 1,Bring referrals, contacts and outcomes together
 ...
 -- 	,r.ReferClosReason
 	...
 	,a.Der_AssTable
 	,a.Der_AssToolCompDate
 -- 	,DATEADD(MONTH, DATEDIFF(MONTH, 0, Der_AssToolCompDate), 0) AS Ass_MonthYear removed as not used later in code and can't do datediff with integers in DAE
 	,a.CodedAssToolType
 ...
 WHERE PersScore <> 'N' AND PersScore <> 'OK' --- remove pesky non-numeric scores -----NEED TO REVIEW THIS AS THE UDF is_numeric might already exclude these PersScores
 -- AND r.RN = 1 ---Removed from TB code 10/05/22
...
# DBTITLE 1,Get all LAST Assessments (RN = 1 is first Assessment). Paired OM has to be on or after date of second contact
 ...
 WHERE a.RN > 1 and a.Der_AssToolCompDate >=a.Contact2 ---needed to COALESCE AssToolCompDate and AssToolCompDateTimestamp for v4.1 to v5
 -- AND r.RN = 1 ---Removed from TB code 10/05/22 a
...
# DBTITLE 1,Match First Assessments with Last Assessments
 ...
 LEFT JOIN $db_output.cyp_partition_assessments a ON r.Person_ID = a.Person_ID AND r.UniqServReqID = a.UniqServReqID AND a.RN = 1 --- join to get first OM ---Removed from TB code 10/05/22
