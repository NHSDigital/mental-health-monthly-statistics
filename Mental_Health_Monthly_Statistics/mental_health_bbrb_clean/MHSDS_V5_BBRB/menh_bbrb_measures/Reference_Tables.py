...
# COMMAND ----------
 ...
 %sql
 DROP TABLE IF EXISTS $db_output.mhsds_org_daily_in_year;
 CREATE TABLE IF NOT EXISTS $db_output.mhsds_org_daily_in_year USING DELTA AS
 SELECT DISTINCT ORG_CODE, 
                 NAME
            FROM (SELECT DISTINCT ORG_CODE, 
                 NAME,
...
# COMMAND ----------
 ...
 CREATE TABLE IF NOT EXISTS $db_output.bbrb_org_daily_latest_mhsds_providers AS
 ...
 INNER JOIN $db_output.mhsds_org_daily od on h.OrgIDProvider = od.ORG_CODE

# COMMAND ----------

 %sql
 DROP TABLE IF EXISTS $db_output.bbrb_org_daily_past_12_months_mhsds_providers;                                 
 CREATE TABLE IF NOT EXISTS $db_output.bbrb_org_daily_past_12_months_mhsds_providers AS
 SELECT DISTINCT od.ORG_CODE, 
                 od.NAME
 ...
 INNER JOIN $db_output.mhsds_org_daily_in_year od on h.OrgIDProvider = od.ORG_CODE

# COMMAND ----------

 %sql
 DROP TABLE IF EXISTS $db_output.bbrb_org_daily_past_quarter_mhsds_providers;                                 
 CREATE TABLE IF NOT EXISTS $db_output.bbrb_org_daily_quarter_months_mhsds_providers AS
 SELECT DISTINCT od.ORG_CODE, 
                 od.NAME
...
# COMMAND ----------
 ...
 where year_of_count = (select max(year_of_count) from $reference_data.ons_population_v2 where GEOGRAPHIC_GROUP_CODE = 'E38')
...
# COMMAND ----------
 ...
 and pop_v2.year_of_count = (select max(year_of_count) from $reference_data.ons_population_v2 where GEOGRAPHIC_GROUP_CODE = 'E38')
...
# COMMAND ----------
 ...
 %sql
 DROP TABLE IF EXISTS $db_output.menh_output_final;
 CREATE TABLE IF NOT EXISTS $db_output.menh_output_final
 ( 
   REPORTING_PERIOD_START DATE,
   REPORTING_PERIOD_END DATE,
   STATUS STRING,
